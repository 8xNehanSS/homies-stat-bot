...
module.exports = {
    ...
    User.find({ guildID: message.guild.id }).sort([['wmessages','descending']]).exec((err,res) => {
        let embed = new Discord.MessageEmbed().setTitle("**Weekly Activity Rank**").setColor(config.color).setFooter("Weekly Ranking").setTimestamp()
        var rankss =  res.length;
        for(i = 0; i < res.length; i++){
            if(res[i].userID === member2){
                const rank = i + 1;
                embed.setDescription(`<@${res[i].userID}> is ranked **${ordinal(rank)}** out of **${guild.rankss12}** weekly members!`)
            }
        }
